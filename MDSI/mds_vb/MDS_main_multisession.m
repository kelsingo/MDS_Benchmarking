...
addpath(genpath('./MDS_combined_model/'));
...
project_path = '';
data_path = '';
result_path = sprintf('results/%s', subj_id);
...
for subj = 1:length(data_subjects{1}.data)
    ...
    cnt = 1;
    for s = 1:S
        Vm = data_subjects{s}.data(subj).Vm;
        J = size(Vm,2);
